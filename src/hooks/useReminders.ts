...
export function useReminders() {
  ...
  useEffect(() => {
    const tick = setInterval(() => {
      ...
      if (st.waterReminderEnabled && now - lastWater.current >= st.waterReminderMin * 60_000) {
        lastWater.current = now
        notify('💧 Time to hydrate', 'Take a sip of water to stay on track.')
        if (st.soundEnabled) playChime('alert')
      }
      if (st.stretchReminderEnabled && now - lastStretch.current >= st.stretchReminderMin * 60_000) {
        lastStretch.current = now
        notify('🧘 Time to wake up & stretch', 'Stand up, move around, and loosen up for a minute.')
        if (st.soundEnabled) playChime('alert')
      }

      // task reminders
      for (const t of useTasks.getState().tasks) {
        if (t.reminderAt && !t.reminderFired && !t.completed && new Date(t.reminderAt).getTime() <= now) {
      ...
      for (const a of useAppointments.getState().appointments) {
        ...
        if (fireAt <= now && new Date(a.start).getTime() >= now - 60_000) {
          ...
          notify('📅 ' + a.title, a.reminderLead ? `Starts in ${a.reminderLead} min` : 'Starting now')
          if (st.soundEnabled) playChime('alert')
        }
      }
    }, 15_000)
